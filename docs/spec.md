...
| **RNF-04 — Eficiência / green code** | O código evita alocações desnecessárias no caminho quente (sem LINQ supérfluo em laços quentes, uso de `Span`/`StringBuilder` onde relevante), usa I/O assíncrona ponta a ponta, *pooling* de conexões do Npgsql, leitura sem *tracking* (`AsNoTracking`) e projeções para evitar N+1, paginação nas listagens e *Server GC* no contêiner. O objetivo é demonstrar uso consciente de recursos e do coletor de lixo. |
